...
public class CharacterController : MonoBehaviour
{
    ...
    public Transform collectibleMainObject;
    ...
    void FixedUpdate()
    {
        ...
        else //animatorun motoruna yapilan ayarlar
        {
    ...
    private void Movement() //karakterin mouse yonunde hareket etmesini saglayan kod. mobile icin yazilsaydi input system devreye girecekti 
    {
    ...
    private void OnTriggerEnter(Collider target)
    {
        ...
        {
            target.transform.SetParent(collectibleMainObject);
            ...
            //character cube stacklemesi icin gereken kod.
        ...
        if (cubes.Count > 1 && target.gameObject.tag == "DizR" || cubes.Count > 1 && target.gameObject.tag != "Diz" + transform.GetChild(0).GetChild(0).GetChild(1).GetComponent<SkinnedMeshRenderer>().material.name.Substring(0, 1) && target.gameObject.tag.StartsWith("Diz"))
        {
            ...
            target.tag = "Diz" + transform.GetChild(0).GetChild(0).GetChild(1).GetComponent<SkinnedMeshRenderer>().name.Substring(0, 1);
